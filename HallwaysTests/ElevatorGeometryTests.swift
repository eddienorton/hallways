...
struct ElevatorGeometryTests {
    ...
    @Test func squareCabKeepsNarrowDoorsAndCenteredPosters() throws {
        ...
        #expect((doors.left.geometry as? SCNBox)?.height == 2.05)
        #expect(abs(doors.left.position.y - 1.025) < 0.00001)
        #expect(abs((3.2 - HallwayScene.ElevatorGeometry(cellSize: 3.2).doorWidth) / 2 - 1.12) < 0.00001)
        let header = try #require(result.scene.rootNode.childNode(withName: "elevatorExteriorHeader", recursively: true))
        #expect(abs(header.position.y - 2.17) < 0.00001)
        #expect((header.geometry as? SCNBox)?.height == 0.24)
        let number = try #require(result.scene.rootNode.childNode(withName: "elevatorExteriorFloorNumber", recursively: true)?.geometry as? SCNText)
        #expect(number.string as? String == "1")
        #expect(abs(shaft.position.y - 1.3) < 0.00001)
        #expect(abs((back.geometry as! SCNBox).height - 2.6) < 0.00001)
