    ...
    var body: some View {
        VStack {
            ...
            if controller.reachedEnd {
                label("You made it!", systemImage: "checkmark.seal.fill")
            } else {
...
struct HallwaySceneView: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> TouchTrackingSCNView {
        ...
        if mazeStore.cells.isEmpty {
            ...
            let (scene, cameraNode, _, wallMaterials, floorMaterial, ceilingMaterial, deadEndCapMaterials) = HallwayScene.build(fromMaze: mazeStore.cells, cellSize: mazeStore.cellSize, wallHeight: mazeStore.wallHeight, objects: mazeStore.objects, theme: themeStore.current)
            view.scene = scene
            view.pointOfView = cameraNode
            ...
            let navController = TapNavigationController(cameraNode: cameraNode, scene: scene, cells: mazeStore.cells, cellSize: mazeStore.cellSize, startCell: start, startFacing: facing, endCell: end)
            // Reaching this floor's end cell — the future elevator —
            // moves on to whatever floor this one's linked to (plain
            // hard cut for now, no doors/animation yet; see
            // MazeStore.advanceToNextMaze). Nulling the controller
            // first avoids a stale "You made it!" flash from the
            // about-to-be-replaced scene — same idea as the grid
            // editor's onDismiss fix.
